...
boolean setMotorTorque(uint8_t id, float torque) {
  ...
  if(id == 1 || id == 3) {
    ...
    current = (int)((torque / MX64Kt) +100) / 3.36; //Calculate current from torque (in milliamperes)
    ...
    current = (int)((torque / MX106Kt) +100) / 3.36; //Calculate current from torque (in milliamperes)
    dxl.writeControlTableItem(GOAL_CURRENT, id, current);
    return 1;
  }
  else {
